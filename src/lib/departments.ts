type Department = {
  label: string;
  value: string;
};

const departments: Department[] = [
  { label: '간호학부', value: '간호학부' },
  { label: '건설환경공학과', value: '건설환경공학과' },
  { label: '건축공학부', value: '건축공학부' },
  { label: '건축학부', value: '건축학부' },
  { label: '경영학부', value: '경영학부' },
  { label: '경제금융학부', value: '경제금융학부' },
  { label: '관광학부', value: '관광학부' },
  { label: '관현악과', value: '관현악과' },
  { label: '교육공학과', value: '교육공학과' },
  { label: '교육학과', value: '교육학과' },
  { label: '국악과', value: '국악과' },
  { label: '국어교육과', value: '국어교육과' },
  { label: '국어국문학과', value: '국어국문학과' },
  { label: '국제학부', value: '국제학부' },
  { label: '기계공학부', value: '기계공학부' },
  { label: '데이터사이언스학과', value: '데이터사이언스학과' },
  { label: '도시공학과', value: '도시공학과' },
  { label: '독어독문학과', value: '독어독문학과' },
  { label: '무용학과', value: '무용학과' },
  { label: '물리학과', value: '물리학과' },
  { label: '미디어커뮤니케이션학과', value: '미디어커뮤니케이션학과' },
  { label: '미래자동차공학과', value: '미래자동차공학과' },
  { label: '사학과', value: '사학과' },
  { label: '사회학과', value: '사회학과' },
  { label: '산업공학과', value: '산업공학과' },
  { label: '산업융합학부', value: '산업융합학부' },
  { label: '생명공학과', value: '생명공학과' },
  { label: '성악과', value: '성악과' },
  { label: '수학과', value: '수학과' },
  { label: '수학교육과', value: '수학교육과' },
  { label: '수행인문학부', value: '수행인문학부' },
  { label: '스포츠산업학과', value: '스포츠산업학과' },
  { label: '식품영양학과', value: '식품영양학과' },
  { label: '신소재공학부', value: '신소재공학부' },
  { label: '실내건축디자인학과', value: '실내건축디자인학과' },
  { label: '심리뇌과학과', value: '심리뇌과학과' },
  { label: '에너지공학과', value: '에너지공학과' },
  { label: '연극영화학과', value: '연극영화학과' },
  { label: '영어교육과', value: '영어교육과' },
  { label: '영어영문학과', value: '영어영문학과' },
  { label: '원자력공학과', value: '원자력공학과' },
  { label: '유기나노공학과', value: '유기나노공학과' },
  { label: '융합전자공학부', value: '융합전자공학부' },
  { label: '응용미술교육과', value: '응용미술교육과' },
  { label: '의류학과', value: '의류학과' },
  { label: '의예과', value: '의예과' },
  { label: '의학과', value: '의학과' },
  { label: '인텔리전스컴퓨팅학부', value: '인텔리전스컴퓨팅학부' },
  { label: '자원환경공학과', value: '자원환경공학과' },
  { label: '작곡과', value: '작곡과' },
  { label: '전기·생체공학부', value: '전기·생체공학부' },
  { label: '정보시스템학과', value: '정보시스템학과' },
  { label: '정책학과', value: '정책학과' },
  { label: '정치외교학과', value: '정치외교학과' },
  { label: '중어중문학과', value: '중어중문학과' },
  { label: '철학과', value: '철학과' },
  { label: '체육학과', value: '체육학과' },
  { label: '컴퓨터소프트웨어학부', value: '컴퓨터소프트웨어학부' },
  { label: '파이낸스경영학과', value: '파이낸스경영학과' },
  { label: '피아노과', value: '피아노과' },
  { label: '행정학과', value: '행정학과' },
  { label: '화학공학과', value: '화학공학과' },
  { label: '화학과', value: '화학과' },
];
export default departments;
